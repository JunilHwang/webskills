<div class="review-write">
    ...
    <form action="<?php echo "{$param->get_page}/update/{$param->idx}"?>" method="post" onsubmit="submitContents(this); return false;" enctype="multipart/form-data">
        <input type="hidden" name="action" value="update">
        <input type="hidden" name="beforeFile" value="<?php echo $data->saved?>">
        <ul class="row s12">
            <li class="col s3 input-field">
                <select name="destination">
                    <option value="" disabled>관광지 선택</option>
                    <?php
                        foreach ($destination as $dest) {
                            $sel = $data->destination == $data->destination ? ' selected' : '';
                    ?>
                        <option value="<?php echo $dest->idx?>"<?php echo $sel?>><?php echo $dest->subject?></option>
                    <?php } ?>
                </select>
            </li>
            <li class="col s9 input-field">
                <input id="review_subject" name="subject" type="text" class="validate" value="<?php echo $data->subject?>" required>
                <label for="review_subject" class="active">제목</label>
            </li>
